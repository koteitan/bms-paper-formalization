...
text \<open>
  Note (2026-05-17): the previously-conjectured lemma
  \<open>BMS_all_B0_ascending_below_t\<close> (= "every B_0 col ascends at all
  rows \<open>k < t\<close>") is FALSE. yaBMS-confirmed counterexample:
  \<open>(0,0,0)(1,1,1)(2,0,0)(1,1,1)\<close> is standard BMS but col 2 does not
  ascend at row 1 (max_parent_level = 2). Its corollary
  \<open>bump_col_uniform_k_lt_t\<close> (uniform bumping at all \<open>k < t\<close>) is
  similarly false. Both lemmas have been deleted. Sound replacements
  are the per-col helpers @{thm bump_col_no_bump} (non-ascending case)
  and \<open>bump_col_at_ascending_row\<close> (ascending case, both in
  BMS_Defs.thy), used with Hunter's per-clause case-split.
\<close>
    ...
    proof (intro allI impI)
      ...
      next
        ...
        \<comment> \<open>Bumped region \<open>k \<ge> l0 + l1\<close> (blocks \<open>B\<^sub>1, \<dots>, B_n\<close>):
            row-0 value is \<open>elem A (s+j) 0 + (if ascends then t\<cdot>delta\<^sub>0 else 0)\<close>;
            remaining crux, co-extensive with consecutive-increase.\<close>
        show ?thesis sorry
      qed
    qed
  qed
