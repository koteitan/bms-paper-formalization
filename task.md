  ...
  - 🚨 **Lemma 2.5**: 5 clauses ancestry — layered per-clause induction
    ...
    - 🚨 **Stage 1: ∀k. (ii)@k** `lemma_2_5_ii_main_v2` (k-induction wrapper、 provides **IH(ii)**)
      ...
      - 🚨 **残 honest sorry 2本に集約 (clean named kernel、 共に t≤mpl 下で真、 probe 0 viol)**:
        ...
        2. `elem_increase_below_suc_parent` — `Suc t'≤mpl`、 p=(Suc t')-parent of q ⟹ **(p,q] の隣接列が全 row k≤t' で狭義増加** `elem(E)(y-1)k<elem(E)y k` (probe 0/23157)。 これが Case B の唯一の純 elem 残カーネル。 そこから上は全て proven scaffolding で結線: `consec_from_elem_increase`(レベル帰納、 `m_parent_zero_pred`/`m_parent_suc_pred` 経由で連続性 `m_parent(E) t' y=y-1`)→ `m_ancestor_of_consecutive_chain`(chain walk で interior=q の t'-祖先)→ `m_ancestor_interior_of_suc_parent` → Case B vacuity。 = Hunter 組合せ核心 (bumped row 構造、 delta/ascends で隣接列増加を示す)
