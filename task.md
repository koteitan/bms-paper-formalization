...
- 🚨 **Theorem 2.7**: BMS は整礎
  ...
  - 🚨 **Lemma 2.5**: 5 clauses ancestry — layered per-clause induction
    ...
    - 🚨 **Stage 1: ∀k. (ii)@k** `lemma_2_5_ii_main_v2` (k-induction wrapper、 provides **IH(ii)**)
      ...
      - 🚨 `bms_b0_col_elem_lt` — **(ii) の唯一の残 crux**: s は全 level r≤t で B_0 row-r 要素の狭義最小 `elem A s r < elem A (s+j) r` (0<j<l1)。 261 BMS 0 viol。 **証明経路特定 (derivation-length 帰納、 BMS.induct 構造保存ではない)**: A=A'[n] で R1 (l1'≥2: bad root=l0'+c0·l1'、 closed-form + delta'>0[proven] + IH で簡約) / R2 (l1'=1: bad root が G' 内、 IH vacuous、 G'-region 議論要) に分岐。 次の sub-crux = **b0_start(A'[n]) 位置補題** (2 regime、 これが naive 構造保存 refuted の正体)。 詳細は lemma 上の doc + verify/probe_bad_root_in_expansion 等
