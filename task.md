...
| 68 | Hunter clause (iv) 攻撃計画 | (b') を Isabelle で formal lemma 化 (sorry 付き)、 これを使って clause (iv) at k<t を内部 dispatch、 残りの k≥t case は別途。 (b') 証明は seed 基本 case vacuous、 induct case A[k] で structural lemmas (b0_start, max_parent_level の expand 後 form) が必要 | 計画段階 | 未着手 | 1-3 sessions |
