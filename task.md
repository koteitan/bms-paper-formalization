...
- 🚨 **Theorem 2.7**: BMS は整礎
  ...
  - 🚨 **Lemma 2.5**: 5 clauses ancestry — layered per-clause induction
    ...
    - 🚨 **Stage 1: ∀k. (ii)@k** `lemma_2_5_ii_main_v2` (k-induction wrapper、 provides **IH(ii)**)
      ...
      - 🚨🚨🚨 **致命的発見 (2026-05-24、Isabelle `eval` で確定): `bms_b0_col_elem_lt` (elem_lt) は形式化で偽**。 反例 E=`(0,0)(1,1)(2,0)(1,1)(1,1)` (列表現 `[[0,0],[1,1],[2,0],[1,1],[1,1]]`)。 eval: `b0_start E=Some 0`, `max_parent_level E=Some 1`, `l1 E=4` ⟹ E は elem_lt の全 hyp を満たす (s=0,t=1,j=2,r=1)。 だが `elem E 0 1=0=elem E 2 1` ⟹ 結論 `elem E 0 1<elem E 2 1` **偽**。 さらに eval: `G_block P @ Bs_concat P 1 = E` (P=`(0,0)(1,1)(2,0)(1,1)(2,0)`, strip 恒等) ⟹ **E∈BMS** (P は seed 到達)。 → **elem_lt は証明不能 (偽だから)。 これが複数セッションの「既約の壁」の真因**。 UNIFIED も偽 (elem_lt はその instance)。 機械検証 witness = BMS_Ancestry 末尾 `cex_elem_lt_*`/`cex_E_is_expansion`。 **root cause 仮説: b0_start/max_parent_level の形式化が Hunter の bad-root 定義と不一致 (退化 b0_start=0)**。 **次セッション最優先 = Hunter 論文と b0_start/max_parent_level 定義の照合**。 現 UNIFIED edifice (ef3e191〜848705f) は偽前提ゆえ要再検討
      - ⚠️ (旧) `bms_b0_col_elem_lt` — `elem A s r < elem A (s+j) r` (r≤t, 0<j<l1)。 UNIFIED から proven 化したが UNIFIED 自体が偽と判明 (上記)
