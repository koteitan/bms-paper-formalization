...
- 🚨 **Theorem 2.7**: BMS は整礎
  ...
  - 🚨 **Lemma 2.5**: 5 clauses ancestry — layered per-clause induction
    ...
    - 🚨 **Stage 1: ∀k. (ii)@k** `lemma_2_5_ii_main_v2` (k-induction wrapper、 provides **IH(ii)**)
      ...
      - ✅ **clause (ii) を honest sorry に復帰** (commit 943fdff): `lemma_2_5_ii_main_v2` は elem_lt 経由(case-B vacuity)で偽依存だった → sorry 化。 clause (ii) 自体は Hunter §2 で真 (証明は同時 k-induction)。 偽連鎖 (UNIFIED/elem_lt/r_ancestor_all/t_ancestor/scaffolding) は dead 化し `[PROVEN-FALSE/DEAD]` マーカー付与 (BMS_Ancestry:5690付近)。 物理削除は follow-up (scattered + comment 内 @{thm} 参照多数ゆえ慎重に; tag v0.1.47 で保全)。
